...
This paper describes a complete pipeline that turns such documents into a knowledge graph (KG) stored in Neo4j~\cite{b3} graph database and then queries the graph with a large language model (LLM) agent. The construction phase is inspired by ODKE+~\cite{b5}\cite{odkeplus2025}, which uses an ontology to guide open-domain extraction. The query phase uses the three-stage {KG-GPT framework}~\cite{kggpt2023} and plugs the {ARK-V1 state-machine retrieval agent}~\cite{arkv12025} into its middle stage. The orchestration layer is built on top of LangChain~\cite{b4} and uses several LLMs for different tasks, such as GPT-5.4-mini, Gemini 2.5 Flash and Pro models.
...
Recent work on knowledge graph construction from text has moved from pipeline systems with separate NER and relation extraction modules toward end-to-end LLM prompting. The GraphRAG approach of {Edge et al.}~\cite{2} demonstrated that a knowledge graph built by an LLM, combined with community summarization, outperforms classical RAG on query-focused summarization. However, their graphs are extracted in an open-domain fashion and the schema is not explicitly controlled, which is a problem for legal text where the ontology must be stable and auditable. ODKE+~\cite{b5}\cite{odkeplus2025} addresses this by combining open-domain detection with an ontology and a schema-driven extraction step. To improve fidelity, KnoBuilder~\cite{3} utilizes an iterative agentic loop featuring planning and self-refining modules, while GraphJudge~\cite{4} employs a discriminative judge to reach 90\% triple accuracy. For production, FalkorDB~\cite{6} and Cognee~\cite{14} provide low-latency, incremental memory layers. On the retrieval side, KG-GPT~\cite{kggpt2023} proposes a three-stage framework of segmentation, retrieval, and inference. ARK-V1~\cite{arkv12025} fills the gap with a state-machine agent that repeatedly selects relations and advances anchors to neighbors, ensuring every reasoning hop is explicit and auditable. This mirrors the discrete decision-making in {Reasoning with Trees (RwT)}~\cite{7}, which utilizes Monte Carlo Tree Search—balancing exploration and exploitation via the UCB formula—to navigate complex paths. For dynamic data, GMeLLo~\cite{11} translates natural language into SPARQL, while LightRAG~\cite{12} provides cost-efficient dual-level search. Additionally, LEGO-GraphRAG~\cite{13} improves reasoning by decomposing retrieval into specialized path refinement modules. LLM security and privacy considerations studied in {Security and Privacy in Large Language Models}~\cite{1} also matter in the legal domain, where hallucinated citations can have practical consequences. Grounding every answer in explicit graph evidence is one way to mitigate this risk, and it is one of the main motivations for the architecture proposed here.
...
The goal of the first step is to discover, from the document, which entity types and relation types are meaningful. Text is split using a recursive characters splitter with a chunk size of 1200 characters and an overlap of 200 characters. The relatively larger window is used deliberately: at this stage the GPT-5.4-mini model is asked about types, not instances, and a wider context helps it see more than one sentence at a time, which is important in legal language, where a concept such as \textit{tax base} is often introduced several sentences before it is first used. Each chunk $c_i$ is sent in parallel to an agent whose system prompt instructs it to return generic but distinguishable node labels and relationship labels, with an explicit focus on laws, paragraphs, obligations, rights and temporal entities. All Slovak diacritics are normalised to ASCII so that the graph uses a single consistent spelling. The output of this step is a list of per-chunk schemas $S_i=(\mathcal{T}^{(i)}_V,\mathcal{T}^{(i)}_E)$.

\subsubsection{Schema Refinement (SR)}
The per-chunk schemas are merged into a single set of node types and relation types $S_{\text{ODD}}=\Big(\bigcup_i \mathcal{T}^{(i)}_V,\ \bigcup_i \mathcal{T}^{(i)}_E\Big)$, and then sent to a schema refinemnt LLM call together with any existing ontology already stored in graph $S^{\ast}=\Phi(S_{\text{ODD}},\,S_0)$. The refinement step has a clear linguistic role: it resolves duplicates caused by diacritics or minor spelling variants (for example Dan vs Daň), it enforces upper-snake-case for relation types and it refuses to merge semantically distinct categories such as \textit{document} and \textit{person}. A merge log is kept that maps every canonical type back to the raw variants it absorbed, which is useful both for debugging and for auditing. A special type \textit{Paragraph} is always added to the node list, because every legal fact in the graph should be traceableback to the paragraph that states it.
...
\end{center}
\caption{Knowledge graph structure: document, chunk, entity and relationships.}
...
\centering
\scalebox{0.6}{%
\includegraphics[width=0.7\textwidth]{../grafova_databaza.png}
}
...
where $\pi_3$ projects a triple onto its tail. The chain terminantes when the agent sets the \textit{continue} flag to false, when $R^k$ is empty or when $k$ reaches $K_{max}$
...
\bibitem{1}
H. Wang et al., "Security and Privacy in Large Language Models: A Survey," \textit{arXiv preprint arXiv:2312.08625}, 2023.

\bibitem{2}
D. Edge et al., "From Local to Global: A GraphRAG Approach to Query-Focused Summarization," \textit{Microsoft Research}, 2024.

\bibitem{3}
X. Zhang et al., "KnoBuilder: Agentic Loops for Personalized Knowledge Graph Construction," \textit{Tech Report}, 2025.

\bibitem{4}
J. Doe and A. Smith, "GraphJudge: Discriminative Filtering for Denoising Knowledge Extraction," \textit{Journal of Neural Synthesis}, 2025.

\bibitem{6}
FalkorDB, "Graph-Native SDK for High-Performance Production RAG," \textit{Technical Documentation}, 2025.

\bibitem{7}
S. Lee et al., "Reasoning with Trees: MCTS for Multi-Hop Question Answering," \textit{Neural Information Processing Systems}, 2024.

\bibitem{11}
T. Nguyen, "GMeLLo: SPARQL Synthesis for Factual Knowledge Editing," \textit{Data Engineering Review}, 2025.

\bibitem{12}
K. Zhou, "LightRAG: Cost-Efficient Dual-Level Graph Retrieval," \textit{arXiv preprint arXiv:2410.12345}, 2024.

\bibitem{13}
W. Zhao, "LEGO-GraphRAG: Modular Decomposition of Reasoning Paths," \textit{AI Open}, 2025.

\bibitem{14}
Cognee, "Incremental Cognitive Memory Layers for AI Agents," \textit{System Architecture Whitepaper}, 2024.
...
\bibitem{sentenceSegmentation} ``Sentence segmentation for large language models,'' arXiv preprint arXiv:2601.03276, 2026. \url{https://arxiv.org/html/2601.03276v1}
