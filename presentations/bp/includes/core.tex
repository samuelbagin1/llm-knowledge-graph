...
\
V kontexte spracovania prirodzeného jazyka prostredníctvom LLM predstavuje štruktúrovaný výstup zásadnú techniku, ktorá zaručuje, že odpoveď volania modelu bude zodpovedať definovanej schéme. Vďaka tomu sa eliminuje chybovosť, ktorú zapričuňuje parsovanie výstupných dát modelu. Vďaka tomuto môžme jednoducho deserializovať dáta do objektov (dictionary, json alebo class), ktoré následne preberá funkcia na ďaľšie spracovanie alebo ich využije. V prostredí knižnice LangChain existují dva odlišné spôsoby, ktoré majú svoje špecifické vlastnosti, výhody a obmedzenia v závislosti od použitého modelu.

\

Prvý prístup, ktorý sa v kóde uplatňuje predovšetkým na modely od spoločnosti OpenAI, je stratégia poskytovateľa (natívny prístup pre OpenAI modely). Táto stratégia funguje na úrovni vyššej abstrakcie, konkrétne u agentového rozhrania. Schéma očakavaného výstupu sa pri tomto prístupe odovzdáva ako parameter formátu odpovede priamo pri vytváraní agenta. Samotná validácia prebieha na strane poskytovateľa. Server poskytovateľa modelu je informovaný o presnej štruktúre, ktorú ma model spĺňať a model je obmedzovaný počas generovania odpovede takzvaným \textit{constrained decodingom}. Tento spôsob využíva natívnu podboru štruktúrovaného výstupu, ktorá zaručuje 100\% zhodu s požadovanou schémou.

Hlavnou výhodou tejto stratégie je deterministická záruka, že vrátený objekt bude vždy validný voči zadanej schéme. Ďaľším významným prínosom je integrácia s agentovým ekosystémom, čo umožňuje plynulé rozšírenie o nástroje, kontextovú pamäť a iteratívne uvažovanie bez nutnosti meniť spôsob, akým sa štruktúrovaný výstup spracováva. V kóde projektu sa tento pristup uplatňuje pri kľúčových úlohach, akými sú detekcia v otvorenej doméne a schémou riadená extrakcia. Jeho nevýhodou je závislosť na konkrétnej implementácií poskytovateľa, modely od iných spoločnosti ako je OpenAI, tento prístup nemusia podporovať.

\ 

Druhý prístup, použitý v projekte pri komunikácií s modelmy Gemini, je metóda priameho štruktúrovaného výstupu. Tento prístup pracuje na nižšej úrovni abstrakcie a úplne obchádza agentovú vrstvu. Funguje tak, že existujúceho klienta modelu obalí do nového objektu. Od volania je konfigurovaný tak, aby vrátil odpoveď zodpovedajúcej schéme. V kóde sa tento princíp kombinuje s metódou založenou na prevode schémy do formátu JSON Schema.

Pri modeloch Gemini je tento prístup obvzlášť efektívny, pretože tieto modely natívne podporujú generovanie odpovedí podľa špecifikácie JSON Schema na úrovni samotného inferenčného jadra. Nie je teda potrebné riešiť problém na úrovni promptu ani spoliehať sa na následnú validáciu.

Výhodou tohto prístupu je jeho jednoduchosť volania a priamy návrat výsledku bez potreby extrahovania z vnoreného kontextového výstupu. To je obzvlášť prínosné v situáciach, kde je extrakcia štruktúrovaných dát atomická operácia, ktorá sa v rámci reťazca opakuje veľmi často. V kóde sa tento prístup uplatňuje pri spracovávaní tabuliek do HTML, transformácií HTML tabuliek do grafovej reprezentácie a pri úprave schémy.
